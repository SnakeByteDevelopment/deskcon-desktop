...
class CTypesData(object, metaclass=CTypesType):
...
class CTypesBackend(object):
    ...
    def complete_struct_or_union(self, CTypesStructOrUnion, fields, tp,
        ...
        def initialize(blob, init):
            ...
            if not isinstance(init, dict):
                ...
                init = dict(list(zip(fnames, init)))
            addr = ctypes.addressof(blob)
            for fname, value in list(init.items()):
        ...
        name2fieldtype = dict(list(zip(fnames, list(zip(btypes, bitfields)))))
        #
        for fname, BField, bitsize in fields:
            if fname == '':
    ...
    def new_enum_type(self, name, enumerators, enumvalues, CTypesInt):
        ...
        reverse_mapping = dict(list(zip(reversed(enumvalues),
