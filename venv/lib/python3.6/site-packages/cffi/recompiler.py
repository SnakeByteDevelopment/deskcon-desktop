...
class Recompiler:
    ...
    def collect_type_table(self):
        ...
        assert None not in list(self._typesdict.values())
        #
    ...
    def _generate(self, step_name):
        lst = list(self.ffi._parser._declarations.items())
    ...
    def collect_step_tables(self):
        ...
        for tp, i in list(self._struct_unions.items()):
            assert i < len(lst)
            assert lst[i].name == tp.name
        assert len(lst) == len(self._struct_unions)
        # same with enums
        lst = self._lsts["enum"]
        for tp, i in list(self._enums.items()):
            assert i < len(lst)
            assert lst[i].name == tp.name
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        typeindex2type = dict([(i, tp) for (tp, i) in list(self._typesdict.items())])
